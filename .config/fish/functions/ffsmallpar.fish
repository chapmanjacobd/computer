...
function ffsmallpar
    ...
    ssh -fN backup
    cat $tmpfile | timeout -s HUP 20h parallel --filter-hosts --sshloginfile ~/.parallel/sshloginfile.ffmpeg --transfer "ffmpeg -nostdin -hide_banner -dn -y -i {} -vf 'scale=-2:min(ih\,1440)' -vcodec libx265 -preset 4 -acodec libopus -b:a 96k {.}.small.mkv && rm {} && rsync -auh --remove-sent-files {.}.small.mkv" $hostname:(pwd)
