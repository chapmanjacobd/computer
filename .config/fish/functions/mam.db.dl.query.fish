...
function mam.db.dl.query --argument db
    ...
    if confirm
        ...
        lb.dev fs "$db" -w 'my_snatched=0' 'seeders>0' 'vip=1' $argv[2..-1] -pf | sqlite.update "$db" media 'my_snatched=1,time_downloaded=unixepoch()' -v
        ...
        lb.dev fs "$db" -w 'my_snatched=0' 'seeders>0' 'vip=0' $argv[2..-1] -pf | sqlite.update "$db" media 'my_snatched=1,time_downloaded=unixepoch()' -v
    end
end
