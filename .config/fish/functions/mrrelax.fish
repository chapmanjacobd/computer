function mrrelax
    ...
    rsync -a --remove-source-files --backup-dir (date "+%d.%m.%Y") --files-from=(
        lt --db ~/lb/tax_sounds.db -s /mnt/d/sync/porn/image/ -p f --moved /mnt/d/sync/porn/image/ /mnt/d/ | psub
    ) /mnt/d/sync/porn/image/ /mnt/d/

    rsync -a --remove-source-files --files-from=(
        lt --db ~/lb/tax_sounds.db -L 24 -p wf --moved /mnt/d/ /mnt/d/sync/porn/image/ | psub
    ) /mnt/d/ /mnt/d/sync/porn/image/

    lb fsadd --video ~/lb/tax.db /mnt/d/sync/porn/image/
