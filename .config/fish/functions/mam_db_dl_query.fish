...
function mam_db_dl_query --argument db
    ...
    lb fs "$db" -w 'my_snatched=0' 'seeders>0' 'vip=1' $argv[2..-1] -pa
    ...
    if confirm
        ...
        lb-dev fs "$db" -w 'my_snatched=0' 'seeders>0' $argv[2..-1] -pf | sqlite_update.py "$db" media 'my_snatched=1,time_downloaded=unixepoch()' -v
    end
end
