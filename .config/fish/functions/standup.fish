...
function standup
    if grep -qEi "(DisplayPort-0)" (kscreen-doctor -o | psub)
        kscreen-doctor output.DisplayPort-0.disable output.HDMI-A-1.disable output.DVI-D-1.disable
        kscreen-doctor output.DVI-D-1.enable output.DVI-D-1.rotation.right output.DVI-D-1.mode.1920x1080@60
    else
