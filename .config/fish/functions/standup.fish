...
function standup
    if grep -qEi "(DisplayPort-0)" (kscreen-doctor -o | psub)
        ...
        kscreen-doctor output.DVI-D-0.enable output.DVI-D-0.rotation.right output.DVI-D-1.mode.1920x1080@60
    else
        kscreen-doctor output.DP-1.disable output.HDMI-1.disable output.DVI-D-0.disable
        kscreen-doctor output.DVI-D-0.enable output.DVI-D-0.rotation.right output.DVI-D-1.mode.1920x1080@50
    end
    krohnkite_off
