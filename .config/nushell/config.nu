...
let abbreviations = {
    ...
    abbrs: 'code ~/.config/nushell/
        code --add ~/.config/nushell/config.nu'
    gp: 'git pull'
    gs: 'git status'
    ga: 'git add .'
    grm: 'git restore'
...
source ~/.config/nushell/jalon-git.nu
...
def defsave [name] {
    view source $name | add_newline | save --append ~/.config/nushell/config.nu
}

def --wrapped winopenall [...argv: string] {
    for $it in (fd ...$argv) {
        if ($it | is-not-empty) {
            print $it
            cmd.exe /C start $it
...
def scoopinstall [ ...packages:string] {
    for $package in $packages {
        ...
        if $exit_status.exit_code == 0 {
            echo $package | add_newline | save --append ~/.github/scoop_installed.txt
        }
    }
}
