...
def savebin [] {
    cp -u ~/bin/nu/nushell_config.nu $nu.config-path
    cp -u ~/.gitconfig ~/bin/User/
    cp -u ~/.inputrc ~/bin/User/
    cp -u ~/.nanorc ~/bin/User/
    cp -u ~/.wezterm.lua ~/bin/User/
}
...
def defsave [name] {
    view source $name | add_newline | save --append ~/bin/nu/nushell_config.nu
}

def --wrapped openall [...argv: string] {
    for $it in (fd ...$argv) {
        if ($it | is-not-empty) {
            print $it
            cmd.exe /C start $it
