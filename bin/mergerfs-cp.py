...
def copy_with_reflink(source, destination):
    subprocess.run(['cp', '--reflink=always', source, destination], check=True)
...
def get_destination_mount(destination):
    ...
    for mergerfs_mount in mergerfs_mounts:
        ...
        if len(common_path) > len(longest_common_path):
...
def main():
    ...
    parser.add_argument('sources', nargs='+', help='Source files to copy.')
    ...
    for source in args.sources:
        ...
        for srcmount in srcmounts:
            original_path = os.path.join(srcmount, os.path.relpath(source, mergerfs_mount))
            if os.path.exists(original_path):
                found_file = True
                dest_path = os.path.join(args.destination, os.path.relpath(source, mergerfs_mount))
                os.makedirs(os.path.dirname(dest_path), exist_ok=True)
                copy_with_reflink(original_path, dest_path)
